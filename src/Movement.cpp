...
void Movement::ProcessInput(GLFWwindow* window, float deltaTime) {
    ...
    for (const auto& line : lineDefs) {
        ...
        if (distSq < collisionRadius * collisionRadius) {
            // Potential collision. Check if it's actually blocking (Height check)
            ...
            if (!blocking) {
                ...
                if (otherSideIdx != -1) {
                    ...
                    // If height difference is too high, it's a wall
                    if (otherFloor - curFloor > MAX_STEP_HEIGHT) blocking = true;
                    // If ceiling is too low, it's a wall
                    if (otherCeil - otherFloor < 0.50f) blocking = true; // Player height approx 0.5 (scaled)
                }
            }

            if (blocking) {
                // Wall sliding via projection
                float wallX = dx, wallZ = dz;
                float wallLen = std::sqrt(lenSq);
                float ux = wallX / wallLen, uz = wallZ / wallLen;

                // Project moveVec onto wall unit vector
                float dot = (moveVec.x * 100.0f * ux + moveVec.z * 100.0f * uz);
                nextPos.x = oldPos.x + (dot * ux) * 0.01f;
                nextPos.z = oldPos.z + (dot * uz) * 0.01f;
                
                // Update local loop vars for next line checks
                px = nextPos.x * 100.0f;
                pz = nextPos.z * 100.0f;
    ...
    // Keep Q/E for vertical movement (debug/spectator style) - overrides lerp if used
